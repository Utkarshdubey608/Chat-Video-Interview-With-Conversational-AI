...
import { FILLER_WORDS, countFillers, calcWpm, type TranscriptEntry } from '@/services/deepgram'
...
export function buildGeminiInput(input: BuilderInput): GeminiAnalysisInput {
  ...
  const questions: QuestionAnswerInput[] = input.questions.map((questionText, idx) => {
    ...
    // Real per-answer duration from THIS question's entry timestamps — never an
    // even split of the whole interview across all configured questions.
    const qTimes = entries.map(e => e.timestamp).filter(Boolean)
    const durationSeconds = qTimes.length >= 2 ? Math.max(0, (Math.max(...qTimes) - Math.min(...qTimes)) / 1000) : 0
  ...
  return {
    ...
    questions,
    overallTranscript,
    // Prefer the caller's derived WPM; recompute from the transcript when 0 so
    // Gemini is never told "0 WPM" as a fact for a real spoken session.
    overallWPM: input.wpm > 0 ? input.wpm : calcWpm(input.transcript),
    overallFillers,
    overallFillerRate,
    humeTopEmotions,
