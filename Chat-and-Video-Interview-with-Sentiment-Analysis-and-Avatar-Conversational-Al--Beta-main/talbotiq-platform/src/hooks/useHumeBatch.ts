...
export function useHumeSubmit(trigger: boolean) {
  ...
  useEffect(() => {
    // No client key needed — submission goes through the server proxy, which
    // holds the Hume key (and falls back to Gemini voice analysis).
    if (!trigger) return
...
export function useHumePoll() {
  ...
  useEffect(() => {
    ...
    // Nothing to poll. NOTE: polling needs no client key — it goes through the
    // server proxy (/api/avatar/hume/*); gating on a key here left completed
    // jobs unpolled and the Results spinner stuck forever.
    if (!jobId) return
    ...
    pollRef.current = setInterval(async () => {
      ...
      try {
        ...
        if (job.status === 'COMPLETED') {
          clearInterval(pollRef.current!)
          // Set the RESULT before flipping status to COMPLETED. The Results
          // page's Gemini ATS auto-run gates on "not processing" — publishing
          // COMPLETED first opened a gap where the scorecard was generated
          // WITHOUT emotion data. And if the predictions fetch dies here, mark
          // FAILED (the old code left the spinner stuck forever).
          try {
            ...
            store.setHumeJobStatus('COMPLETED')
            toast.success('Emotion analysis complete', { duration: 3000 })
          } catch (err) {
            console.error('[HumePoll] predictions fetch/build failed:', err)
            store.setHumeJobStatus('FAILED')
          }
        } else if (job.status === 'FAILED') {
          clearInterval(pollRef.current!)
          store.setHumeJobStatus('FAILED')
          toast.error('Voice-emotion analysis failed — see console for details', { duration: 6000 })
        } else {
